...
float opacity;
int milliTimer;// for milliseconds --> seconds
int milliTimerM;// for milliseconds --> minutes
int milliMove; // milliTimer - millis
int milliMoveM; // milliTimerM - millis
int min;
int counter = 0;
...
void setup() {
  size(900, 750);
  noStroke();
  mapImage = loadImage("sfmap.jpg");

milliTimer = millis() + 1000;
milliTimerM = millis() + 59000;
...
//count through rows of location table, 
  //for (int row = 0; row<rowCount; row++) {
    ...
    String id = amountsTable.getRowName(counter);
    ...
    drawData(x, y, id);
  //}
   milliMove = milliTimer - millis();
   
   if (milliTimer < millis()) {
    //remap the value to a range between 1.5 and 15 
    milliTimer = milliTimer + 1000;
    counter++;
  }
if (milliMove < 500){
    opacity = map(milliMove, 0, 500, 0, 255);
    
    //and make it this color
    fill(HSB, hour()*10, 255, opacity);
  
  } else {
    //otherwise, if the number is negative, make it this color.
   opacity = map(milliMove, 500, 1000, 255, 0);
...
//if the value variable holds a float greater than or equal to 0
    ...
    radius = map(value, 0, dataMin, 0.5, 15); 
...
//grey circle
milliMoveM = milliTimerM - millis();
 
  if (milliTimerM < millis()) {
    milliTimerM = milliTimerM + 59000;
  }
  if (milliMoveM < 59000/2) {
    opacity = map(milliMoveM, 0, 59000/2, 0, 255);
  } else {
    opacity = map(milliMoveM, 59000/2, 59000, 255, 0);
  }
  fill(255, opacity);
  ellipse(646, 146, 45, 45);
  
  fill (255, opacity);
  ellipse(415, 635, 80, 80);
  
  fill (255, opacity);
  ellipse(366, 97, 60, 60);
  
  fill (255, opacity);
  ellipse(517, 259, 40, 40);
  
  fill (255, opacity);
  ellipse(394, 233, 30, 30);
}